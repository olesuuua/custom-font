...
"""Reconcile saved QA decisions into the live v4 report and checkpoint."""
...
def main() -> int:
    ...
    if rejected:
        raise RuntimeError("structurally blocked manual passes: " + ", ".join(rejected))

    report_tmp = rf.REPORT.with_suffix(rf.REPORT.suffix + ".tmp")
    with report_tmp.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(rows)
        handle.flush()
    report_tmp.replace(rf.REPORT)

    shutil.copy2(rf.REPORT, v4.V4 / "redraw-report.csv")
    shutil.copy2(rf.DECISIONS, v4.V4 / "redraw-manual-decisions.json")

    manifest_path = v4.V4 / "manifest.json"

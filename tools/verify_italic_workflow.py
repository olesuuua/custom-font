...
from import_italic_xopp import DEFAULT_FFPYTHON, extract_glyph_svgs, run_fontforge_worker, validate_svg_pages
...
def load_and_verify_manifest() -> dict:
    ...
    for name in font.getGlyphOrder():
        ...
        (actual_drawable if pen.bounds else actual_empty).append(name)
    font.close()
    if set(names) != set(actual_drawable):
        errors.append("manifest glyphs differ from the source outline inventory")
    if tuple(actual_empty) != EMPTY_GLYPHS:
        errors.append("source empty glyph inventory differs")
    if errors:
...
def make_synthetic_svg_pages(directory: Path):
    ...
    for page in range(1, PAGE_COUNT + 1):
        (directory / f"page-{page:02d}.svg").write_text(
            ...
            f'viewBox="0 0 {A4_WIDTH} {A4_HEIGHT}"><path d="M10 10L20 20" fill="none" stroke="#000000"/></svg>',
            encoding="utf-8",
        )
...
def synthetic_round_trip(manifest: dict, full_fontforge: bool) -> dict:
    with tempfile.TemporaryDirectory(prefix="italic-workflow-") as temporary:
        ...
        make_synthetic_svg_pages(svg_pages)
        extraction = extract_glyph_svgs(xopp, manifest, temporary_path / "expanded")
        ...
        if full_fontforge:
            ...
            if len(font.getGlyphOrder()) != 340:
                raise AssertionError("synthetic italic font does not preserve 340 glyph slots")
...
def main():
    ...
    print(json.dumps({"status": "passed", "manifest_glyphs": 334, "pdf_pages": PAGE_COUNT, "synthetic": synthetic}, indent=2))

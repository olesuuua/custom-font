...
"""Verify the isolated Italic v8 redraw, fidelity report, and QA wiring."""
...
def main() -> int:
    required = [v8.OUTPUT_SFD, v8.OUTPUT_TTF, v8.REPORT, v8.MANIFEST,
                v8.FAMILY_REPORT, v8.DECISIONS, ROOT / "qa" / "italic.html"]
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise RuntimeError("missing Italic v8 artifacts: " + " ".join(missing))
    manifest = json.loads(v8.MANIFEST.read_text(encoding="utf-8"))
    with v8.REPORT.open(encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    by_name = {row["glyph"]: row for row in rows}
    source = fontforge.open(str(v8.SOURCE_SFD))
    output = fontforge.open(str(v8.OUTPUT_SFD))
    ttf = fontforge.open(str(v8.OUTPUT_TTF))
    predecessor = fontforge.open(str(v8.V7_BASE_SFD))
    errors = []
    try:
        ...
        for path, expected in manifest["immutable_hashes"].items():
            ...
            if not actual_path.exists() or v8.sha256(actual_path) != expected:
        ...
        expected_changed = {"S", "six", "uni0412", "Beta", "uni0416", "uni042A"}
        actual_changed = {
        ...
        if actual_changed != expected_changed:
            errors.append(
                "v8 changed slots differ: " + ",".join(sorted(actual_changed))
            )
        if rf.layer_hash(output["Beta"].foreground) != rf.layer_hash(output["uni0412"].foreground):
            errors.append("Beta and uni0412 do not share the exact v8 outline")
        for name in ("S", "six"):
        ...
        if "v2 source" not in page or "v8 TTF outline" not in page:
            errors.append("QA page is not the required v2/v8 comparison")
        if "italic-v8-glyph-report.csv" not in server or "italic-v8-manual-decisions.json" not in server:
            errors.append("QA server is not wired to the v8 report and decisions")
    finally:
        source.close(); output.close(); ttf.close(); predecessor.close()
    print(json.dumps({
        "glyphs": len(rows),
        "editable_points": sum(int(row["candidate_points"]) for row in rows),

...
"""Build the isolated, smooth, low-point Italic v8 review candidate.

Run with FontForge's Python runtime:

    fontforge -lang=py -script tools/redraw_italic_v8.py --workers 8
    fontforge -lang=py -script tools/redraw_italic_v8.py --glyph A
...
V7_BASE_SFD = ROOT / "fontforge" / "italic-v7-redraw-review.sfd"
REGULAR_FINAL_SFD = ROOT / "regular-final.sfd"
OUTPUT_SFD = ROOT / "fontforge" / "italic-v8-redraw-review.sfd"
OUTPUT_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v8-redraw-review.ttf"
FINAL_SFD = ROOT / "fontforge" / "italic-v8.sfd"
FINAL_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v8.ttf"
WORK = ROOT / "output" / "italic-v8-redraw"
REPORT = WORK / "italic-v8-glyph-report.csv"
MANIFEST = WORK / "manifest.json"
FAMILY_SOURCE = ROOT / "tools" / "italic_v8_families.json"
FAMILY_REPORT = WORK / "family-registry.json"
DECISIONS = ROOT / "qa" / "italic-v8-manual-decisions.json"
...
CANDIDATE_SVG_DIR = WORK / "v8-final-font-glyph-svg"
...
ALGORITHM_CACHE_VERSION = "hybrid-centerline-protected-detail-v8-a5-local-joins"
...
def v8_refit_candidate(baseline_layer, allocation, point_sets, context, method,
...
def v8_shared_beta_candidate(v6_font, point_sets, context):
    return evaluate_custom(
        point_sets, context, v6_font["uni0412"].foreground.dup(),
        "v8-shared-Beta-uni0412-canonical",
    )


def v8_s_candidate(source_glyph, source_layer, point_sets, context):
    candidate = visible_structure_candidate(
        source_glyph, source_layer, point_sets, context,
    )
    if candidate is not None:
        candidate["method"] = "v8-S-visible-structure-terminal-preserving"
    return candidate


def v8_zhe_candidate(v6_font, point_sets, context):
    ...
    return evaluate_custom(
        ...
        "v8-Zhe-local-junction-pinch-protected-corners",
    )
...
def v8_uni042a_candidate(v6_font, point_sets, context):
    ...
    return evaluate_custom(
        point_sets, context, layer,
        "v8-uni042A-top-loop-local-handle-alignment",
    )
...
def shared_b_middle_candidate(source_font, point_sets, context):
    candidate = protected_detail_candidate(
        ...
        method="v8-shared-B-adaptive-protected-middle-detail",
    )
    ...
    return evaluate_custom(
        point_sets, context, layer,
        "v8-shared-B-protected-middle-cubic-join",
    )
...
def zhe_exaggerated_junction_candidate(v7_font, point_sets, context):
    ...
    return evaluate_custom(
        point_sets, context, layer,
        "v8-Zhe-separated-protected-junction-corners",
    )
...
def v8_yeru_candidate(v6_font, point_sets, context):
    ...
    for contour in fitted:
        layer += contour.dup()
    return evaluate_custom(
        point_sets, context, layer, "v8-Yeru-right-stem-from-l",
    )
    ...
    for name in review_names:
        ...
        for operation in ("flip-x", "flip-y", "rotate-180"):
            ...
            if name not in targets and signature in signatures:
                ...
                break
    return {
        "version": "olesuas-hand-italic-v8-family-registry-v1",
...
def worker_build(name: str, destination_sfd: Path, destination_json: Path) -> int:
    ...
    v7_base = fontforge.open(str(V7_BASE_SFD))
    ...
    try:
        ...
        baseline = evaluate_custom(
            point_sets, context, v7_base[name].foreground.dup(),
            "v7-reviewed-baseline",
        )
        ...
        if name in {"Beta", "uni0412"}:
            family_spec = {
                ...
                "donor": "uni0412",
                "transform": "adaptive-refit+protected-middle-detail",
            }
            family = shared_b_middle_candidate(source, point_sets, context)
            family_status = "user-directed-reuse"
        elif name == "S":
            family_spec = {
                "family": "v8-centerline-stroke-rebuild", "donor": "XOPP-S-strokes",
                "transform": "smooth-centerline+coupled-uniform-width",
            }
            family = centerline_stroke_candidate(
                "S", point_sets, context,
                segments=(3, 10), passes=(3, 5),
                protected=(((505, 675),), ((500, 680), (175, 260))),
                method="v8-S-centerline-coupled-uniform-width",
            )
            family_status = "user-directed-exception"
        elif name == "six":
            family_spec = {
                "family": "v8-centerline-stroke-rebuild", "donor": "XOPP-six-stroke",
                "transform": "smooth-centerline+coupled-uniform-width",
            }
            family = centerline_stroke_candidate(
                ...
                method="v8-six-centerline-coupled-uniform-width",
            )
            family_status = "user-directed-exception"
        elif name == "uni0416":
            family_spec = {
                "family": "v8-protected-detail-repair", "donor": "v7-uni0416",
                "transform": "separated-junction+protected-corner-nodes",
            }
            family = zhe_exaggerated_junction_candidate(
                v7_base, point_sets, context,
            )
            family_status = "user-directed-exception"
        elif name == "uni042A":
            family_spec = {
                "family": "v8-protected-detail-refit", "donor": "v2-uni042A",
            ...
            family = protected_detail_candidate(
                ...
                method="v8-uni042A-adaptive-protected-top-detail",
            )
            family_status = "user-directed-exception"
        elif name == "uni042B":
            family_spec = {
                "family": "v8-Yeru-composition", "donor": "uni042B-left,l-right",
                "transform": "preserve-left+registered-l",
            }
            family = v8_yeru_candidate(v7_base, point_sets, context)
            family_status = "user-directed-reuse"
        elif name == "D":
            family_spec = {
            ...
            family = degree_candidate(source, v7_base, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "Phi":
            family_spec = {
            ...
            family = angstrom_candidate(source, v7_base, point_sets, context)
            family_status = "user-directed-reuse"
        ...
        if review_mode == SMOOTH_REVIEW:
            smoothed = baseline_smoothing_candidate(
                v7_base[name].foreground.dup(), point_sets, context, name,
            )
...
def build(selected_names: set[str], workers: int) -> int:
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
        ...
        for future in as_completed(futures):
            ...
            if complete == 1 or complete % 10 == 0 or complete == len(names):
                print(f"italic v8 redraw: {complete}/{len(names)}", flush=True)
    ...
    if not DECISIONS.exists():
        atomic_json(DECISIONS, {"version": "italic-v8-manual-review-v1", "decisions": {}})
    with REPORT.open(encoding="utf-8-sig", newline="") as handle:
    ...
    if immutable_after != immutable_before:
        raise RuntimeError("an immutable source artifact changed during the v8 build")
    ...
    manifest = {
        "format": "olesuas-hand-italic-v8-redraw-v1",
...
def finalize() -> int:
    if not REPORT.exists() or not OUTPUT_SFD.exists() or not OUTPUT_TTF.exists():
        raise RuntimeError("build and review Italic v8 before finalization")
    ...
    if unresolved:
        raise RuntimeError(
            f"Italic v8 review is incomplete or stale for {len(unresolved)} glyphs; "
            f"first: {unresolved[:12]}"
        )
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if manifest["point_reduction"] < .70:
        raise RuntimeError("Italic v8 does not meet the 70% point-reduction release gate")

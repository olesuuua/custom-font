...
"""Verify the isolated Italic v7 redraw, fidelity report, and QA wiring."""
...
def main() -> int:
    required = [v7.OUTPUT_SFD, v7.OUTPUT_TTF, v7.REPORT, v7.MANIFEST,
                v7.FAMILY_REPORT, v7.DECISIONS, ROOT / "qa" / "italic.html"]
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise RuntimeError("missing Italic v7 artifacts: " + " ".join(missing))
    manifest = json.loads(v7.MANIFEST.read_text(encoding="utf-8"))
    with v7.REPORT.open(encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    by_name = {row["glyph"]: row for row in rows}
    source = fontforge.open(str(v7.SOURCE_SFD))
    output = fontforge.open(str(v7.OUTPUT_SFD))
    ttf = fontforge.open(str(v7.OUTPUT_TTF))
    errors = []
    try:
        ...
        for path, expected in manifest["immutable_hashes"].items():
            ...
            if not actual_path.exists() or v7.sha256(actual_path) != expected:
        ...
        if "v2 source" not in page or "v7 TTF outline" not in page:
            errors.append("QA page is not the required v2/v7 comparison")
        if "italic-v7-glyph-report.csv" not in server or "italic-v7-manual-decisions.json" not in server:
            errors.append("QA server is not wired to the v7 report and decisions")
    finally:

...
"""Verify the isolated Italic v6 redraw, fidelity report, and QA wiring."""
...
def main() -> int:
    required = [v6.OUTPUT_SFD, v6.OUTPUT_TTF, v6.REPORT, v6.MANIFEST,
                v6.FAMILY_REPORT, v6.DECISIONS, ROOT / "qa" / "italic.html"]
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise RuntimeError("missing Italic v6 artifacts: " + " ".join(missing))
    manifest = json.loads(v6.MANIFEST.read_text(encoding="utf-8"))
    with v6.REPORT.open(encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    by_name = {row["glyph"]: row for row in rows}
    source = fontforge.open(str(v6.SOURCE_SFD))
    output = fontforge.open(str(v6.OUTPUT_SFD))
    ttf = fontforge.open(str(v6.OUTPUT_TTF))
    errors = []
    try:
        ...
        for path, expected in manifest["immutable_hashes"].items():
            ...
            if not actual_path.exists() or v6.sha256(actual_path) != expected:
        ...
        if "v2 source" not in page or "v6 TTF outline" not in page:
            errors.append("QA page is not the required v2/v6 comparison")
        if "italic-v6-glyph-report.csv" not in server or "italic-v6-manual-decisions.json" not in server:
            errors.append("QA server is not wired to the v6 report and decisions")
    finally:

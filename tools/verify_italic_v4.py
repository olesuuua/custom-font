...
"""Verify the isolated Italic v4 redraw, fidelity report, and QA wiring."""
...
def main() -> int:
    required = [v4.OUTPUT_SFD, v4.OUTPUT_TTF, v4.REPORT, v4.MANIFEST,
                v4.FAMILY_REPORT, v4.DECISIONS, ROOT / "qa" / "italic.html"]
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise RuntimeError("missing Italic v4 artifacts: " + " ".join(missing))
    manifest = json.loads(v4.MANIFEST.read_text(encoding="utf-8"))
    with v4.REPORT.open(encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    by_name = {row["glyph"]: row for row in rows}
    source = fontforge.open(str(v4.SOURCE_SFD))
    output = fontforge.open(str(v4.OUTPUT_SFD))
    ttf = fontforge.open(str(v4.OUTPUT_TTF))
    errors = []
    try:
        ...
        for path, expected in manifest["immutable_hashes"].items():
            ...
            if not actual_path.exists() or v4.sha256(actual_path) != expected:
        ...
        if "v2 source" not in page or "v4 TTF outline" not in page:
            errors.append("QA page is not the required v2/v4 comparison")
        if "italic-v4-glyph-report.csv" not in server or "italic-v4-manual-decisions.json" not in server:

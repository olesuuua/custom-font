...
"""Build the isolated, smooth, low-point Italic v5 review candidate.

Run with FontForge's Python runtime:

    fontforge -lang=py -script tools/redraw_italic_v5.py --workers 8
    fontforge -lang=py -script tools/redraw_italic_v5.py --glyph A
    fontforge -lang=py -script tools/redraw_italic_v5.py --finalize
...
V4_BASE_SFD = ROOT / "fontforge" / "italic-v4-redraw-review.sfd"
REGULAR_FINAL_SFD = ROOT / "regular-final.sfd"
OUTPUT_SFD = ROOT / "fontforge" / "italic-v5-redraw-review.sfd"
OUTPUT_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v5-redraw-review.ttf"
FINAL_SFD = ROOT / "fontforge" / "italic-v5.sfd"
FINAL_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v5.ttf"
WORK = ROOT / "output" / "italic-v5-redraw"
REPORT = WORK / "italic-v5-glyph-report.csv"
MANIFEST = WORK / "manifest.json"
FAMILY_SOURCE = ROOT / "tools" / "italic_v5_families.json"
FAMILY_REPORT = WORK / "family-registry.json"
DECISIONS = ROOT / "qa" / "italic-v5-manual-decisions.json"
TEMPLATE_MANIFEST = ROOT / "output" / "italic-template" / "italic-template-manifest.json"
SOURCE_SVG_DIR = WORK / "v2-source-glyph-svg"
CANDIDATE_SVG_DIR = WORK / "v5-final-font-glyph-svg"
...
ALGORITHM_CACHE_VERSION = "review-guided-cubic-v13-structural-fidelity"
...
REVIEW_MODE_OVERRIDES = {"D": PURE_SMOOTH_REVIEW}
    ...
    for name in review_names:
        ...
        for operation in ("flip-x", "flip-y", "rotate-180"):
            ...
            if name not in targets and signature in signatures:
                ...
                break
    return {
        "version": "olesuas-hand-italic-v5-family-registry-v1",
...
def worker_build(name: str, destination_sfd: Path, destination_json: Path) -> int:
    ...
    v4_base = fontforge.open(str(V4_BASE_SFD))
    ...
    try:
        ...
        if review_mode == DETAIL_REVIEW:
            baseline = evaluate_custom(
                point_sets, context, v4_base[name].foreground.dup(),
                "v4-rework-baseline-guard",
            )
        ...
        if name == "D":
            family_spec = {
                "family": "user-directed-smooth-letter", "donor": "v4-D-shape",
                "transform": "preserve-geometry+reclassify-smooth-nodes",
            }
            family = evaluate_custom(
                point_sets, context, v4_base[name].foreground.dup(),
                "user-v4-D-preserved-smooth-node-reclassification",
            )
            ...
            family = degree_candidate(source, v4_base, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "Phi":
            family_spec = {
            ...
            family = angstrom_candidate(source, v4_base, point_sets, context)
            family_status = "user-directed-reuse"
...
def build(selected_names: set[str], workers: int) -> int:
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
        ...
        for future in as_completed(futures):
            ...
            if complete == 1 or complete % 10 == 0 or complete == len(names):
                print(f"italic v5 redraw: {complete}/{len(names)}", flush=True)
    ...
    if not DECISIONS.exists():
        atomic_json(DECISIONS, {"version": "italic-v5-manual-review-v1", "decisions": {}})
    with REPORT.open(encoding="utf-8-sig", newline="") as handle:
    ...
    if immutable_after != immutable_before:
        raise RuntimeError("an immutable source artifact changed during the v5 build")
    ...
    manifest = {
        "format": "olesuas-hand-italic-v5-redraw-v1",
...
def finalize() -> int:
    if not REPORT.exists() or not OUTPUT_SFD.exists() or not OUTPUT_TTF.exists():
        raise RuntimeError("build and review Italic v5 before finalization")
    ...
    if unresolved:
        raise RuntimeError(
            f"Italic v5 review is incomplete or stale for {len(unresolved)} glyphs; "
            f"first: {unresolved[:12]}"
        )
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if manifest["point_reduction"] < .70:
        raise RuntimeError("Italic v5 does not meet the 70% point-reduction release gate")

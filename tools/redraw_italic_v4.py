...
"""Build the isolated, smooth, low-point Italic v4 review candidate.

Run with FontForge's Python runtime:

    fontforge -lang=py -script tools/redraw_italic_v4.py --workers 8
    fontforge -lang=py -script tools/redraw_italic_v4.py --glyph A
    fontforge -lang=py -script tools/redraw_italic_v4.py --finalize
...
OUTPUT_SFD = ROOT / "fontforge" / "italic-v4-redraw-review.sfd"
OUTPUT_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v4-redraw-review.ttf"
FINAL_SFD = ROOT / "fontforge" / "italic-v4.sfd"
FINAL_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v4.ttf"
WORK = ROOT / "output" / "italic-v4-redraw"
REPORT = WORK / "italic-v4-glyph-report.csv"
MANIFEST = WORK / "manifest.json"
FAMILY_SOURCE = ROOT / "tools" / "italic_v4_families.json"
FAMILY_REPORT = WORK / "family-registry.json"
DECISIONS = ROOT / "qa" / "italic-v4-manual-decisions.json"
TEMPLATE_MANIFEST = ROOT / "output" / "italic-template" / "italic-template-manifest.json"
SOURCE_SVG_DIR = WORK / "v2-source-glyph-svg"
CANDIDATE_SVG_DIR = WORK / "v4-final-font-glyph-svg"
...
ALGORITHM_CACHE_VERSION = "review-guided-cubic-v9"
...
REVIEW_MODE_OVERRIDES = {"K": SMOOTH_REVIEW}
...
def base_candidates(source_layer, review_mode="") -> tuple[dict, list[list[tuple[float, float]]], dict, int]:
    ...
    candidates = uniform + (
        [] if uniform and review_mode in (SMOOTH_REVIEW, DETAIL_REVIEW) else [regular]
    )
    ...
    clean = [
        candidate for candidate in uniform
    ...
    if review_mode == SMOOTH_REVIEW:
        ...
        pool = structurally_valid or clean or regular_clean or passing or uniform
    else:
    ...
    if pool:
        if review_mode == SMOOTH_REVIEW:
            ...
            selected = min(pool, key=lambda value: (
                rf.candidate_key(value), -value["points"],
                value["spacing_cv"], -value["curve_ratio"],
            ))
        else:
            selected = min(pool, key=lambda value: (
...
def forced_quote_candidate(source_font, target_name, point_sets, context):
    donor_source = source_font["quotesinglbase"].foreground.dup()
    donor, _sets, _context, _count = base_candidates(donor_source)
    doubles = target_name in {
        "quotedbl", "quotedblleft", "quotedblright", "quotedblbase", "second",
    }
    orientation = "rotate-180" if target_name in {
        "quoteright", "quotereversed", "quotedblright",
    } else "register"
    boxes = principal_boxes(source_font[target_name].foreground, 2 if doubles else 1)
    layers = [
        uniform_registered_layer(donor["layer"], layer_bbox(donor_source), box, orientation)
        for box in boxes
    ]
    ...
    return evaluate_custom(
        ...
        f"user-family:quotes:quotesinglbase:{orientation}:{len(boxes)}",
    )
...
def zero_candidate(source_layer, point_sets, context):
    boxes = principal_boxes(source_layer, 2)
    boxes = sorted(boxes, key=lambda box: (box[2] - box[0]) * (box[3] - box[1]), reverse=True)
    layer = fontforge.layer()
    layer.is_quadratic = False
    layer += cubic_oval(boxes[0], reverse=False)
    layer += cubic_oval(boxes[1], reverse=True)
    return evaluate_custom(point_sets, context, layer, "user-geometric-smooth-zero")
...
def original_infinity_candidate(point_sets, context):
    ...
    candidates = []
    for budget in (80, 90, 100):
        allocation = allocate_segments(visible, budget // 3)
        if allocation is None:
            continue
        layer, lines, curves = build_uniform_curve_layer(visible, allocation, SMOOTH_REVIEW)
        candidate = rf.evaluate_layer(
    ...
    return evaluate_custom(
        point_sets, context, exact["layer"], "user-original-svg-smooth-infinity",
    )
...
def angstrom_candidate(source_font, point_sets, context):
    ...
    ring_boxes = sorted((contour_bbox(contour) for contour in contours[1:]),
                        key=lambda box: (box[2] - box[0]) * (box[3] - box[1]), reverse=True)
    ring = fontforge.layer()
    ring.is_quadratic = False
    ring += cubic_oval(ring_boxes[0], reverse=False)
    ring += cubic_oval(ring_boxes[1], reverse=True)
    candidate = evaluate_custom(
        point_sets, context, combine_layers([a_layer, ring]),
        "user-composite:uni212B:uniform-scaled-A+circle",
        max_points=1000,
    )
    ...
    for name in review_names:
        ...
        for operation in ("flip-x", "flip-y", "rotate-180"):
            ...
            if name not in targets and signature in signatures:
                ...
                break
    return {
        "version": "olesuas-hand-italic-v4-family-registry-v1",
...
def worker_build(name: str, destination_sfd: Path, destination_json: Path) -> int:
    ...
    try:
        ...
        if name in {
            ...
            "quotedblright", "quotedblbase", "quotedbl", "quotesingle",
            "minute", "second",
        }:
            family_spec = {
                "family": "user-directed-quotes", "donor": "quotesinglbase",
                "transform": "registered-repeat-or-rotation",
            }
            ...
            family_spec = {
                "family": "user-directed-zero", "donor": "geometric-ovals",
                "transform": "source-envelopes",
            }
            ...
            family_spec = {
                "family": "user-directed-angstrom", "donor": "A,circle",
                "transform": "uniform-scale+compose",
            }
            family = angstrom_candidate(source, point_sets, context)
            family_status = "user-directed-reuse"
...
def build(selected_names: set[str], workers: int) -> int:
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
        ...
        for future in as_completed(futures):
            ...
            if complete == 1 or complete % 10 == 0 or complete == len(names):
                print(f"italic v4 redraw: {complete}/{len(names)}", flush=True)
    ...
    if not DECISIONS.exists():
        atomic_json(DECISIONS, {"version": "italic-v4-manual-review-v1", "decisions": {}})
    with REPORT.open(encoding="utf-8-sig", newline="") as handle:
    ...
    if immutable_after != immutable_before:
        raise RuntimeError("an immutable source artifact changed during the v4 build")
    ...
    manifest = {
        "format": "olesuas-hand-italic-v4-redraw-v1",
...
def finalize() -> int:
    if not REPORT.exists() or not OUTPUT_SFD.exists() or not OUTPUT_TTF.exists():
        raise RuntimeError("build and review Italic v4 before finalization")
    ...
    if unresolved:
        raise RuntimeError(
            f"Italic v4 review is incomplete or stale for {len(unresolved)} glyphs; "
            f"first: {unresolved[:12]}"
        )
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if manifest["point_reduction"] < .70:
        raise RuntimeError("Italic v4 does not meet the 70% point-reduction release gate")

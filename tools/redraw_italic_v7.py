...
"""Build the isolated, smooth, low-point Italic v7 review candidate.

Run with FontForge's Python runtime:

    fontforge -lang=py -script tools/redraw_italic_v7.py --workers 8
    fontforge -lang=py -script tools/redraw_italic_v7.py --glyph A
    fontforge -lang=py -script tools/redraw_italic_v7.py --finalize
...
OUTPUT_SFD = ROOT / "fontforge" / "italic-v7-redraw-review.sfd"
OUTPUT_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v7-redraw-review.ttf"
FINAL_SFD = ROOT / "fontforge" / "italic-v7.sfd"
FINAL_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v7.ttf"
WORK = ROOT / "output" / "italic-v7-redraw"
REPORT = WORK / "italic-v7-glyph-report.csv"
MANIFEST = WORK / "manifest.json"
FAMILY_SOURCE = ROOT / "tools" / "italic_v7_families.json"
FAMILY_REPORT = WORK / "family-registry.json"
DECISIONS = ROOT / "qa" / "italic-v7-manual-decisions.json"
TEMPLATE_MANIFEST = ROOT / "output" / "italic-template" / "italic-template-manifest.json"
SOURCE_SVG_DIR = WORK / "v2-source-glyph-svg"
CANDIDATE_SVG_DIR = WORK / "v7-final-font-glyph-svg"
...
ALGORITHM_CACHE_VERSION = "review-guided-cubic-v25-v7-path-preserving-Yeru"
...
REVIEW_MODE_OVERRIDES = {
    ...
    **{name: "" for name in (
        "Beta", "uni0412", "S", "six", "uni0416", "uni042A", "uni042B",
    )},
}
...
def baseline_smoothing_candidate(baseline_layer, point_sets, context, name):
    ...
    for item in candidates:
        ...
        evaluated.append(enrich_candidate(
            value, f"v6-local-pure-cubic-smoothing-{item['points']}",
        ))
    clean = [
...
def v7_refit_candidate(baseline_layer, allocation, point_sets, context, method,
...
def v7_shared_beta_candidate(v6_font, point_sets, context):
    return evaluate_custom(
        point_sets, context, v6_font["uni0412"].foreground.dup(),
        "v7-shared-Beta-uni0412-canonical",
    )


def v7_s_candidate(source_glyph, source_layer, point_sets, context):
    candidate = visible_structure_candidate(
        source_glyph, source_layer, point_sets, context,
    )
    if candidate is not None:
        candidate["method"] = "v7-S-visible-structure-terminal-preserving"
    return candidate


def v7_zhe_candidate(v6_font, point_sets, context):
    ...
    return evaluate_custom(
        ...
        "v7-Zhe-local-junction-pinch-protected-corners",
    )
...
def v7_uni042a_candidate(v6_font, point_sets, context):
    ...
    return evaluate_custom(
        point_sets, context, layer,
        "v7-uni042A-top-loop-local-handle-alignment",
    )
...
def v7_yeru_candidate(v6_font, point_sets, context):
    ...
    for contour in fitted:
        layer += contour.dup()
    return evaluate_custom(
        point_sets, context, layer, "v7-Yeru-right-stem-from-l",
    )
    ...
    for name in review_names:
        ...
        for operation in ("flip-x", "flip-y", "rotate-180"):
            ...
            if name not in targets and signature in signatures:
                ...
                break
    return {
        "version": "olesuas-hand-italic-v7-family-registry-v1",
...
def worker_build(name: str, destination_sfd: Path, destination_json: Path) -> int:
    ...
    try:
        ...
        baseline = evaluate_custom(
            point_sets, context, v6_base[name].foreground.dup(),
            "v6-reviewed-baseline",
        )
        ...
        family = None
        if name in {"Beta", "uni0412"}:
            family_spec = {
                "family": "shared-Greek-Cyrillic-B",
                "donor": "uni0412",
                "transform": "exact-outline-copy",
            }
            family = v7_shared_beta_candidate(v6_base, point_sets, context)
            family_status = "user-directed-reuse"
        elif name == "S":
            family_spec = {
                "family": "v7-local-terminal-repair", "donor": "v2-S-visible-ink",
                "transform": "boolean-clean+detail-preserving-cubic-refit",
            }
            family = v7_s_candidate(glyph, source_layer, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "six":
            family_spec = {
                "family": "v7-local-digit-smoothing", "donor": "v6-six",
                "transform": "outer-curve-long-span-refit",
            }
            family = v7_refit_candidate(
                v6_base[name].foreground.dup(), [22, 5], point_sets, context,
                "v7-six-long-span-outer-smoothing",
            )
            family_status = "user-directed-exception"
        elif name == "uni0416":
            family_spec = {
                "family": "v7-protected-center-detail", "donor": "v2-uni0416",
                "transform": "detail-refit+corner-protection",
            }
            family = v7_zhe_candidate(v6_base, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "uni042A":
            family_spec = {
                "family": "v7-local-top-curve-smoothing", "donor": "v6-uni042A",
                "transform": "long-span-cubic-refit",
            }
            family = v7_uni042a_candidate(v6_base, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "uni042B":
            family_spec = {
                "family": "v7-Yeru-composition", "donor": "uni042B-left,l-right",
                "transform": "preserve-left+registered-l",
            }
            family = v7_yeru_candidate(v6_base, point_sets, context)
            family_status = "user-directed-reuse"
        elif name == "D":
            family_spec = {
                "family": "user-directed-smooth-letter", "donor": "v6-D-shape",
                "transform": "preserve-geometry+reclassify-smooth-nodes",
            }
            family = evaluate_custom(
                point_sets, context, v6_base[name].foreground.dup(),
                "user-v6-D-preserved-smooth-node-reclassification",
            )
            family_status = "user-directed-exception"
        elif name in {
            ...
            family = degree_candidate(source, v6_base, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "Phi":
            family_spec = {
            ...
            family = angstrom_candidate(source, v6_base, point_sets, context)
            family_status = "user-directed-reuse"
        ...
        if review_mode == SMOOTH_REVIEW:
            smoothed = baseline_smoothing_candidate(
                v6_base[name].foreground.dup(), point_sets, context, name,
            )
...
def build(selected_names: set[str], workers: int) -> int:
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
        ...
        for future in as_completed(futures):
            ...
            if complete == 1 or complete % 10 == 0 or complete == len(names):
                print(f"italic v7 redraw: {complete}/{len(names)}", flush=True)
    ...
    if not DECISIONS.exists():
        atomic_json(DECISIONS, {"version": "italic-v7-manual-review-v1", "decisions": {}})
    with REPORT.open(encoding="utf-8-sig", newline="") as handle:
    ...
    if immutable_after != immutable_before:
        raise RuntimeError("an immutable source artifact changed during the v7 build")
    ...
    manifest = {
        "format": "olesuas-hand-italic-v7-redraw-v1",
...
def finalize() -> int:
    if not REPORT.exists() or not OUTPUT_SFD.exists() or not OUTPUT_TTF.exists():
        raise RuntimeError("build and review Italic v7 before finalization")
    ...
    if unresolved:
        raise RuntimeError(
            f"Italic v7 review is incomplete or stale for {len(unresolved)} glyphs; "
            f"first: {unresolved[:12]}"
        )
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if manifest["point_reduction"] < .70:
        raise RuntimeError("Italic v7 does not meet the 70% point-reduction release gate")

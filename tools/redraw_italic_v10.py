...
"""Build the isolated, feedback-directed Italic v10 review candidate.

Run with FontForge's Python runtime:

    fontforge -lang=py -script tools/redraw_italic_v10.py --workers 8
    fontforge -lang=py -script tools/redraw_italic_v10.py --glyph A
    fontforge -lang=py -script tools/redraw_italic_v10.py --finalize

The accepted v2 font and the reviewed v9 candidate are immutable inputs.
Glyphs unchanged in this round are exact v9 copies so their review decisions
carry over. Every candidate stays at or below 100 editable foreground points.
"""
...
PREVIOUS_REPORT = ROOT / "output" / "italic-v9-redraw" / "italic-v9-glyph-report.csv"
SOURCE_SFD = ROOT / "fontforge" / "italic-v2-review.sfd"
SOURCE_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v2-review.ttf"
V9_BASE_SFD = ROOT / "fontforge" / "italic-v9-redraw-review.sfd"
V9_BASE_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v9-redraw-review.ttf"
REVIEWED_SEED_SFD = ROOT / "fontforge" / "italic-v10-redraw-seeds.sfd"
...
OUTPUT_SFD = ROOT / "fontforge" / "italic-v10-redraw-review.sfd"
OUTPUT_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v10-redraw-review.ttf"
FINAL_SFD = ROOT / "fontforge" / "italic-v10.sfd"
FINAL_TTF = ROOT / "output" / "font" / "OlesuasHand-Italic-v10.ttf"
WORK = ROOT / "output" / "italic-v10-redraw"
REPORT = WORK / "italic-v10-glyph-report.csv"
MANIFEST = WORK / "manifest.json"
PROGRESS_METRICS = WORK / "progress-metrics.json"
FAMILY_SOURCE = ROOT / "tools" / "italic_v10_families.json"
FAMILY_REPORT = WORK / "family-registry.json"
DECISIONS = ROOT / "qa" / "italic-v10-manual-decisions.json"
PREVIOUS_DECISIONS = ROOT / "qa" / "italic-v9-manual-decisions.json"
...
CANDIDATE_SVG_DIR = WORK / "v10-final-font-glyph-svg"
...
ALGORITHM_CACHE_VERSION = "v10-reviewed-seed-a1"
...
V10_CHANGED_GLYPHS = {
    "S", "g", "uni0416", "uni0437", "uni0446", "seveneighths",
}
POINT_CEILINGS = {}
    ...
    for name in review_names:
        ...
        for operation in ("flip-x", "flip-y", "rotate-180"):
            ...
            if name not in targets and signature in signatures:
                ...
                break
    return {
        "version": "olesuas-hand-italic-v10-family-registry-v1",
...
ZHE_WEDGES = (
    ((485.0, 470.0), (467.0, 422.0), (428.0, 469.0)),
    ((396.0, 351.0), (415.0, 409.0), (444.0, 351.0)),
    ((469.0, 352.0), (485.0, 395.0), (533.0, 351.0)),
    ((591.0, 470.0), (557.0, 407.0), (509.0, 469.0)),
)
V10_ZHE_COMPACT_NODES = (
    (199.7, 665.8), (107.3, 249.3), (514.9, 94.4), (540.6, 102.1),
)
...
def contour_segments(values):
    on = [i for i, point in enumerate(values) if point.on_curve]
    segments = []
    size = len(values)
    for position, i in enumerate(on):
        j = on[(position + 1) % len(on)]
        segments.append((
            (values[i].x, values[i].y),
            (values[(i + 1) % size].x, values[(i + 1) % size].y),
            (values[(j - 1) % size].x, values[(j - 1) % size].y),
            (values[j].x, values[j].y),
        ))
    return segments


def split_segment(segment, t, position):
    p0, p1, p2, p3 = segment

    def lerp(a, b, u):
        return (a[0] + (b[0] - a[0]) * u, a[1] + (b[1] - a[1]) * u)

    r0 = lerp(p0, p1, t); r1 = lerp(p1, p2, t); r2 = lerp(p2, p3, t)
    s0 = lerp(r0, r1, t); s1 = lerp(r1, r2, t)
    mid = lerp(s0, s1, t)
    if position is not None:
        mid = position
    return (p0, r0, s0, mid), (mid, s1, r2, p3)


def v10_zhe_junction_exception(v9_base, source_layer, point_sets, context):
    """Widen the four crossing notches on the reviewed v9 outline.

    The v9 outline already dips at every notch but its rims are rounded shut.
    This moves each notch apex onto the v2 source apex, splits the wall cubics
    at the source rim corners, and clamps the inserted corner handles so the
    walls stay straight.  Nothing is removed: every other anchor of the
    reviewed v9 outline is preserved verbatim.
    """
    layer = v9_base["uni0416"].foreground.dup()
    main = layer[0]
    values = list(main)
    segments = contour_segments(values)
    corner_handle = 9.0

    for apex_index, (_rim1, apex, _rim2) in enumerate(ZHE_WEDGES):
        best_segment, best_dist = None, 1e9
        for segment_index, segment in enumerate(segments):
            dist = math.dist(segment[0], apex)
            if dist < best_dist:
                best_segment, best_dist = segment_index, dist
        if best_dist < 30:
            segment_index = best_segment
            segment = segments[segment_index]
            segments[segment_index] = (apex, segment[1], segment[2], segment[3])
            previous = segments[(segment_index - 1) % len(segments)]
            segments[(segment_index - 1) % len(segments)] = (
                previous[0], previous[1], previous[2], apex)

    inserted = 0
    for rim1, _apex, rim2 in ZHE_WEDGES:
        for rim in (rim1, rim2):
            best_index, best_t, best_dist = None, 0.0, 1e9
            for index, segment in enumerate(segments):
                for step, point in enumerate(flatten_cubic(*segment)):
                    dist = math.dist(point, rim)
                    if dist < best_dist:
                        best_index, best_t, best_dist = index, step / 24.0, dist
            if best_dist > 25 or best_t < 0.04 or best_t > 0.96:
                continue
            left, right = split_segment(segments[best_index], best_t, rim)
            segments[best_index:best_index + 1] = [left, right]
            inserted += 1

    def near_feature(point):
        features = [p for wedge in ZHE_WEDGES for p in wedge]
        return any(math.dist(point, feature) < 14 for feature in features)

    contour = fontforge.contour()
    contour.is_quadratic = False
    contour.moveTo(*segments[0][0])
    for start, c1, c2, end in segments:
        ...
        if near_feature(start):
            direction = rf.normalize((c1[0] - start[0], c1[1] - start[1]))
            c1 = (start[0] + direction[0] * corner_handle,
                  start[1] + direction[1] * corner_handle)
        if near_feature(end):
            direction = rf.normalize((c2[0] - end[0], c2[1] - end[1]))
            c2 = (end[0] + direction[0] * corner_handle,
                  end[1] + direction[1] * corner_handle)
        if (math.dist(start, end) < 0.5 or
                (line_dist(c1) < 0.8 and line_dist(c2) < 0.8)):
            contour.lineTo(*end)
        else:
            contour.cubicTo(c1, c2, end)
    contour.closed = True
    result = fontforge.layer()
    result.is_quadratic = False
    result += contour
    result += layer[1].dup()
    result = rf.classify_layer(result)
    return evaluate_custom(
        point_sets, context, result, "v10-Zhe-junction-notches-user-cap-exception",
        max_points=POINT_CEILINGS["uni0416"],
    )


def v10_seveneighths_fraction_donor(v9_base, v5_base, point_sets, context):
    """Keep v5's passed seven and slash; adopt the consensus 8 the user passed.

    The three passed fraction glyphs (oneeighth, threeeighths, fiveeighths)
    share one clean consensus 8.  Its contours are registered into the
    seveneighths 8 cell, replacing the defective v5 8 outline wholesale.
    """
    donor_layer = v9_base["threeeighths"].foreground
    v5_layer = v5_base["seveneighths"].foreground.dup()
    if len(donor_layer) != 5 or len(v5_layer) != 5:
        raise RuntimeError("seveneighths contour inventory changed")
    sets = []
    for donor_index, target_index in ((0, 0), (3, 3), (4, 4)):
        registered = rv4.registered_to_bbox(
            single_contour_layer(donor_layer[donor_index].dup()),
            contour_bbox(v5_layer[target_index]),
        )
        sets.append(sf.resample_polyline(
            list(sf.layer_point_sets(registered))[0], 2.0, closed=True))
    ladders = rf.build_ladders(sets, max_points=45, targets=(40, 45))
    layer = None
    for _counts, anchors in ladders:
        layer, _lines, _curves = rf.build_layer(sets, anchors)
    if layer is None:
        raise RuntimeError("v10 seveneighths donor produced no ladder candidate")
    layer = rf.classify_layer(layer)
    ordered = fontforge.layer()
    ordered.is_quadratic = False
    ordered += layer[0]
    ordered += v5_layer[1]
    ordered += v5_layer[2]
    ordered += layer[1]
    ordered += layer[2]
    ordered = rf.classify_layer(ordered)
    candidate = evaluate_custom(
        point_sets, context, ordered,
        "v10-seveneighths-passed-fraction-eight-donor",
    )
    if candidate["points"] > MAX_POINTS:
        raise RuntimeError("v10 seveneighths exceeds the point cap")
    return candidate


def single_contour_layer(contour):
    layer = fontforge.layer()
    layer.is_quadratic = False
    layer += contour
    return layer
...
V10_ZHE_DIPS = (
    {"name": "A", "src_lo": 133, "src_hi": 206, "wrap": (422.0, 443.0)},
    {"name": "D", "src_lo": 4319, "src_hi": 4416, "wrap": (496.0, 428.0)},
    {"name": "B", "src_lo": 2039, "src_hi": 2118, "wrap": (459.0, 395.0)},
    {"name": "C", "src_lo": 2540, "src_hi": 2612, "wrap": (543.0, 377.0)},
)


def contour_segments_from_values(values):
    on = [i for i, point in enumerate(values) if point.on_curve]
    segments = []
    size = len(values)
    for position, i in enumerate(on):
...
def rebuild_contour_from_segments(segments):
    contour = fontforge.contour()
    contour.is_quadratic = False
    contour.moveTo(*segments[0][0])
    for start, c1, c2, end in segments:
        def line_dist(point, start=start, end=end):
            dx, dy = end[0] - start[0], end[1] - start[1]
            length = math.hypot(dx, dy)
            if length < 1e-6:
                return math.dist(point, start)
        ...
        if (math.dist(start, end) < 0.5
                or (line_dist(c1) < 0.8 and line_dist(c2) < 0.8)):
            contour.lineTo(*end)
        else:
            contour.cubicTo(c1, c2, end)
    contour.closed = True
...
def region_source_snap(name, layer, source_layer, boxes, max_shift):
    """Displace anchors inside region boxes onto their nearest source point."""
    source_dense = [p for c in sf.layer_point_sets(source_layer) for p in c]
    ...
    for contour in layer:
        ...
        for point in values:
            p = (point.x, point.y)
            if in_region(p):
                target = min(source_dense, key=lambda s: math.dist(s, p))
                d = (target[0] - p[0], target[1] - p[1])
                dist = math.hypot(*d)
                if dist > max_shift:
                    d = (d[0] * max_shift / dist, d[1] * max_shift / dist)
                deltas.append(d)
            else:
                deltas.append((0.0, 0.0))
        blended = list(deltas)
        on_idx = [i for i, p in enumerate(values) if p.on_curve]
        for k, i in enumerate(on_idx):
            if deltas[i] == (0.0, 0.0):
                continue
            for nb in ((i - 1) % size, (i + 1) % size):
                if not values[nb].on_curve:
                    blended[nb] = deltas[i]
        for point, d in zip(values, blended):
            point.x += d[0]
            point.y += d[1]
    return rf.classify_layer(layer)


def v10_s_source_snap(v9_base, point_sets, context):
    """Refit S from v2 with a smooth 31-anchor terminal-biased cubic."""
    source_layer = _v10_source["S"].foreground.dup()
    return biased_detail_candidate(
        source_layer, point_sets, context,
        allocation=(31,),
        protected_targets=(
            (547, 793), (500, 701), (479, 586), (552, 745),
            (174, 253), (147, 307), (190, 35), (55, 235),
            (110, 120), (60, 290),
        ),
        method="v10-S-smooth-source-terminal-detail",
    )


def v10_seveneighths_own_eight(v5_base, point_sets, context):
    """The v5 seveneighths drawing (correct width) with a smoothed top-right.

    v10's consensus-eight donor read wide with uneven stroke because the
    passed fraction 8 is a different drawing registered into the 7/8 cell.
    v10 returns to the glyph's own v5 outline and only snaps the 8's top-right
    arc onto the source, which removes the bump without touching the stroke
    weight, the seven, the slash, or the counters' budget.
    """
    layer = v5_base["seveneighths"].foreground.dup()
    source_layer = _v10_source["seveneighths"].foreground.dup()
    layer = region_source_snap(
        "seveneighths", layer, source_layer, [(430, 140, 545, 315)], 12.0)
    candidate = evaluate_custom(
        point_sets, context, layer,
        "v10-seveneighths-passed-fraction-eight-donor:own-eight-snap",
    )
    if candidate["points"] > MAX_POINTS:
        raise RuntimeError("v10 seveneighths exceeds the point cap")
    return candidate
...
def v10_g_corner_splits(v9_base, point_sets, context):
    """Use fewer, smoother bowl nodes and reserve the budget for g's crossing."""
    source_layer = _v10_source["g"].foreground.dup()
    return biased_detail_candidate(
        source_layer, point_sets, context,
        allocation=(15, 5, 5, 3, 2, 2, 2),
        protected_targets=(
            (336, 66), (344, 8), (320, 2), (338, -19),
            (314, -25), (458, 19), (285, -65), (250, -110),
        ),
        method="v10-g-smoother-bowl-crossing-detail",
    )


def v10_zhe_dip_transplant(v9_base, point_sets, context):
    """Free five broad-curve nodes while preserving the reviewed central notches."""
    base = v9_base["uni0416"].foreground.dup()
    values = list(base[0])
    on_indices = [i for i, point in enumerate(values) if point.on_curve]
    segments = []
    for position, start_index in enumerate(on_indices):
        end_index = on_indices[(position + 1) % len(on_indices)]
        cursor = (start_index + 1) % len(values)
        controls = []
        while cursor != end_index:
            controls.append((values[cursor].x, values[cursor].y))
            cursor = (cursor + 1) % len(values)
        segments.append({
            "start": (values[start_index].x, values[start_index].y),
            "end": (values[end_index].x, values[end_index].y),
            "controls": controls,
        })

    def flatten(segment, steps=36):
        p0, p3 = segment["start"], segment["end"]
        p1, p2 = segment["controls"]
        return [
            (
                (1 - t) ** 3 * p0[0] + 3 * (1 - t) ** 2 * t * p1[0]
                + 3 * (1 - t) * t ** 2 * p2[0] + t ** 3 * p3[0],
    ...
    def least_squares_controls(samples):
        p0, p3 = samples[0], samples[-1]
        aa = ab = bb = 0.0
        rhs = [[0.0, 0.0], [0.0, 0.0]]
        distances = [0.0]
        ...
        for index, point in enumerate(samples):
            ...
            b = 3.0 * u * t * t
            residual = (
                point[0] - u ** 3 * p0[0] - t ** 3 * p3[0],
                point[1] - u ** 3 * p0[1] - t ** 3 * p3[1],
            )
            aa += a * a
            ab += a * b
            bb += b * b
            for axis in (0, 1):
                rhs[0][axis] += a * residual[axis]
                rhs[1][axis] += b * residual[axis]
        determinant = aa * bb - ab * ab
        if abs(determinant) < 1e-9:
            raise RuntimeError("v10 Ж cubic merge is singular")
        c1 = tuple(
            (rhs[0][axis] * bb - rhs[1][axis] * ab) / determinant
            for axis in (0, 1)
        )
        c2 = tuple(
            (aa * rhs[1][axis] - ab * rhs[0][axis]) / determinant
            for axis in (0, 1)
        )
        return c1, c2

    for target in V10_ZHE_COMPACT_NODES:
        index = min(
            range(1, len(segments)),
            key=lambda item: math.dist(segments[item]["start"], target),
        )
        previous = segments[index - 1]
        current = segments[index]
        if len(previous["controls"]) != 2 or len(current["controls"]) != 2:
            raise RuntimeError(f"v10 Ж broad node {target} borders a line")
        samples = flatten(previous) + flatten(current)[1:]
        control1, control2 = least_squares_controls(samples)
        segments[index - 1] = {
    ...
    redundant_line = min(
        range(1, len(segments)),
        key=lambda item: math.dist(segments[item]["start"], (509.0, 469.0)),
    )
    previous = segments[redundant_line - 1]
    current = segments[redundant_line]
    if previous["controls"] or current["controls"]:
        raise RuntimeError("v10 Ж redundant junction point stopped being linear")
    previous["end"] = current["end"]
    del segments[redundant_line]
    if redundant_line < len(segments):
        segments[redundant_line]["start"] = previous["end"]
    layer = fontforge.layer()
    layer.is_quadratic = False
    contour = fontforge.contour()
    contour.is_quadratic = False
    contour.moveTo(*segments[0]["start"])
    for segment in segments:
        if len(segment["controls"]) == 2:
            contour.cubicTo(
                segment["controls"][0], segment["controls"][1], segment["end"],
            )
        elif not segment["controls"]:
            contour.lineTo(*segment["end"])
        else:
            raise RuntimeError("v10 Ж encountered an invalid segment")
    contour.closed = True
    layer += contour
    if point_count(layer) > MAX_POINTS:
        raise RuntimeError("v10 Ж broad-curve compaction missed the point cap")
    return evaluate_custom(
        point_sets, context, layer, "v10-Zhe-junction-preserving-local-compact",
    )
...
def worker_build(name: str, destination_sfd: Path, destination_json: Path) -> int:
    ...
    _v10_source = source
    v9_base = fontforge.open(str(V9_BASE_SFD))
    reviewed_seed = fontforge.open(str(REVIEWED_SEED_SFD))
    ...
    try:
        ...
        baseline = evaluate_custom(
            point_sets, context, v9_base[name].foreground.dup(),
            "v10-reviewed-baseline",
        )
        if name in V10_CHANGED_GLYPHS:
            selected = evaluate_custom(
                point_sets, context, reviewed_seed[name].foreground.dup(),
                "reviewed-v10-seed",
            )
            row = candidate_row(
                glyph, source_layer, selected, count + 1,
                {
                    "family": "reviewed-v10-candidate",
                    "donor": "reviewed-seed-" + name,
                    "transform": "exact-reviewed-outline",
                },
                "reviewed-seed",
                time.perf_counter() - started,
            )
            save_worker_glyph(glyph, selected["layer"], destination_sfd)
        ...
        if name == "g":
            family_spec = {
                "family": "v10-metric-gated-lowercase-g", "donor": "v2-g",
                "transform": "15-anchor-main+protected-crossing-details",
            }
            family = v10_g_corner_splits(v9_base, point_sets, context)
            ...
            family_spec = {
                "family": "v10-local-detail-small-Tse", "donor": "v2-uni0446",
                "transform": "rebalanced-main-curve+lower-right-join",
            }
            family = biased_detail_candidate(
                source_layer, point_sets, context,
                allocation=(29, 4),
                protected_targets=(
                    (212, 470), (423, 506), (225, 514), (60, 259),
                    (277, 204), (412, 90), (486, 37), (473, 202),
                    (435, 21), (444, -2), (393, -61), (480, -95),
                ),
                method="v10-small-Tse-rebalanced-lower-right-join",
            )
            family_status = "user-directed-exception"
        elif name == "uni044E":
            family_spec = {
            ...
            family_spec = {
                "family": "v10-terminal-biased-S", "donor": "v2-S",
                "transform": "31-anchor-source-refit+protected-terminals",
            }
            family = v10_s_source_snap(v9_base, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "six":
            family_spec = {
            ...
            family_spec = {
                "family": "v10-compact-Zhe", "donor": "reviewed-seed-uni0416",
                "transform": "remove-broad-curve-redundancy+preserve-notches",
            }
            family = v10_zhe_dip_transplant(v9_base, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "seveneighths":
            family_spec = {
                "family": "v10-own-eight-seven-eighths", "donor": "v5-seveneighths",
                "transform": "own-drawing+top-right-source-snap",
            }
            family = v10_seveneighths_own_eight(v5_base, point_sets, context)
            family_status = "user-directed-exception"
        elif name == "uni042A":
            family_spec = {
        ...
        if name in V10_CHANGED_GLYPHS:
            selected = evaluate_custom(
                point_sets, context, reviewed_seed[name].foreground.dup(),
                "reviewed-v10-seed:" + selected.get("method", "candidate"),
            )
            count += 1
        if name not in V10_CHANGED_GLYPHS:
            # v10 is an isolated feedback round. Prior review tags must not
            # silently re-run legacy repair branches on unchanged v10 glyphs.
            selected = baseline
            family_spec = None
            family_status = "v10-preserved-unchanged"
        row = candidate_row(
...
def write_progress_metrics(report_rows: list[dict]) -> dict:
    with PREVIOUS_REPORT.open(encoding="utf-8-sig", newline="") as handle:
        before = {row["glyph"]: row for row in csv.DictReader(handle)}
    after = {row["glyph"]: row for row in report_rows}
    results = {}
    for name in sorted(V10_CHANGED_GLYPHS):
        old, new = before[name], after[name]
        old_p95, new_p95 = float(old["boundary_p95"]), float(new["boundary_p95"])
        old_max, new_max = float(old["boundary_max"]), float(new["boundary_max"])
        old_points, new_points = int(old["candidate_points"]), int(new["candidate_points"])
        old_curves, new_curves = float(old["curve_ratio"]), float(new["curve_ratio"])
        p95_change = new_p95 / old_p95 - 1.0 if old_p95 else 0.0
        max_change = new_max / old_max - 1.0 if old_max else 0.0
        point_change = new_points / old_points - 1.0 if old_points else 0.0
        curve_change = new_curves / old_curves - 1.0 if old_curves else 0.0
        no_structural_regression = (
            int(new["self_intersections"]) == 0
            and int(new["invalid_handles"]) == 0
            and new_points <= MAX_POINTS
        )
        quality_not_worse = p95_change <= .15 and max_change <= .15
        meaningful = no_structural_regression and quality_not_worse and (
            p95_change <= -.10 or max_change <= -.10
            or point_change <= -.10 or curve_change >= .20
        )
        results[name] = {
            "baseline": {
                "version": "v9", "points": old_points,
                "boundary_p95": old_p95, "boundary_max": old_max,
                "curve_ratio": old_curves,
            },
            "candidate": {
                "version": "v10", "points": new_points,
                "boundary_p95": new_p95, "boundary_max": new_max,
                "curve_ratio": new_curves,
                "topology_status": new["topology_status"],
                "self_intersections": int(new["self_intersections"]),
            },
            "relative_change": {
                "points": point_change, "boundary_p95": p95_change,
                "boundary_max": max_change, "curve_ratio": curve_change,
            },
            "meaningful_progress": meaningful,
            "disposition": (
                "improved-for-review" if meaningful and name != "uni0416"
                else "point-cap-recovery-still-needs-review"
            ),
        }
    payload = {
        "format": "italic-v10-vs-v9-progress-v1",
        "changed_glyphs": sorted(V10_CHANGED_GLYPHS),
        "rule": (
            "At least 10% improvement in boundary error or point count, or "
            "20% relative curve-node improvement, with no intersections, invalid "
            "handles, or point-cap violation. Ж remains unresolved despite its cap recovery."
        ),
        "glyphs": results,
    }
    atomic_json(PROGRESS_METRICS, payload)
    return payload
...
def build(selected_names: set[str], workers: int) -> int:
    ...
    if selected_names:
        names = [name for name in all_names if name in selected_names]
    else:
        # Unchanged glyphs are never rebuilt: their v9 outlines, references,
        # and report rows are carried over verbatim so review decisions and
        # TTF outline hashes stay valid without re-review.
        names = [name for name in all_names if name in V10_CHANGED_GLYPHS]
    ...
    if REPORT.exists():
        with REPORT.open(encoding="utf-8-sig", newline="") as handle:
            previous_rows = carried_rows = {row["glyph"]: row for row in csv.DictReader(handle)}
    elif PREVIOUS_REPORT.exists():
        with PREVIOUS_REPORT.open(encoding="utf-8-sig", newline="") as handle:
            carried_rows = {row["glyph"]: row for row in csv.DictReader(handle)}
    carried_names = [name for name in all_names if name not in set(names)]
    results = {}
    with ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
        ...
        for future in as_completed(futures):
            ...
            if complete == 1 or complete % 10 == 0 or complete == len(names):
                print(f"italic v10 redraw: {complete}/{len(names)}", flush=True)
    # Full rebuilds start from the reviewed v10 font; carried glyphs keep
    # their exact v10 foregrounds and only the changed slots are overwritten.
    base_path = OUTPUT_SFD if selected_names and OUTPUT_SFD.exists() else V9_BASE_SFD
    output = fontforge.open(str(base_path))
    rows = []
    try:
        for name in names:
        ...
        # A full rebuild starts from the v9 font itself, so every carried
        # glyph is already byte-identical to v9 in the SFD.  Touching the
        # layer (even a self-assignment) would rebuild the in-memory splines
        # and change the generated TTF outlines, so the font is not modified
        # here at all; only the report rows are carried over.  A partial
        # rebuild leaves the existing v10 outlines alone.
        for name in carried_names:
            if selected_names:
                if name in previous_rows:
                    rows.append(dict(previous_rows[name]))
                continue
            row = dict(carried_rows.get(name, {}))
            if row:
                rows.append(row)
    ...
    if not DECISIONS.exists():
        if PREVIOUS_DECISIONS.exists():
            ...
            atomic_json(DECISIONS, {"version": "italic-v10-manual-review-v1", "decisions": {}})
    with REPORT.open(encoding="utf-8-sig", newline="") as handle:
        report_rows = list(csv.DictReader(handle))
    changed_for_review = selected_names or V10_CHANGED_GLYPHS
    ...
    if immutable_after != immutable_before:
        raise RuntimeError("an immutable source artifact changed during the v10 build")
    ...
    progress = write_progress_metrics(report_rows)
    manifest = {
        "format": "olesuas-hand-italic-v10-redraw-v1",
        ...
        "point_ceilings": {name: POINT_CEILINGS[name] for name in sorted(POINT_CEILINGS)},
        ...
        "artifact_hashes": {
            ...
            str(PROGRESS_METRICS.relative_to(ROOT)): sha256(PROGRESS_METRICS),
        },
        "progress_metrics": str(PROGRESS_METRICS.relative_to(ROOT)),
        "meaningful_progress_glyphs": sum(
            item["meaningful_progress"] for item in progress["glyphs"].values()
        ),
...
def finalize() -> int:
    if not REPORT.exists() or not OUTPUT_SFD.exists() or not OUTPUT_TTF.exists():
        raise RuntimeError("build and review Italic v10 before finalization")
    ...
    over_budget = [
        ...
        if int(row["candidate_points"]) > POINT_CEILINGS.get(row["glyph"], MAX_POINTS)
    ]
    if over_budget:
        raise RuntimeError("glyphs over their point ceiling: " + " ".join(over_budget))
    if unresolved:
        raise RuntimeError(
            f"Italic v10 review is incomplete or stale for {len(unresolved)} glyphs; "
            f"first: {unresolved[:12]}"
        )
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if manifest["point_reduction"] < .70:
        raise RuntimeError("Italic v10 does not meet the 70% point-reduction release gate")
